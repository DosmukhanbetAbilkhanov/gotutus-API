    ...
    public static function form(Schema $form): Schema
    {
        return $form
            ->schema([
                ...
                \Filament\Schemas\Components\Section::make('Activity Types')
                    ->schema([
                        Forms\Components\CheckboxList::make('activityTypes')
                            ->relationship('activityTypes', 'id')
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('name_en')
                    ->label('Name (EN)')
                    ->getStateUsing(fn (Goal $record) => $record->translations->firstWhere('language_code', 'en')?->name)
                    ->searchable(query: function ($query, string $search) {
                        $query->whereHas('translations', fn ($q) => $q->where('language_code', 'en')->where('name', 'like', "%{$search}%"));
                    }),
                Tables\Columns\TextColumn::make('name_ru')
                    ->label('Name (RU)')
                    ->getStateUsing(fn (Goal $record) => $record->translations->firstWhere('language_code', 'ru')?->name)
                    ->searchable(query: function ($query, string $search) {
                        $query->whereHas('translations', fn ($q) => $q->where('language_code', 'ru')->where('name', 'like', "%{$search}%"));
                    }),
                Tables\Columns\TextColumn::make('name_kk')
                    ->label('Name (KK)')
                    ->getStateUsing(fn (Goal $record) => $record->translations->firstWhere('language_code', 'kk')?->name)
                    ->searchable(query: function ($query, string $search) {
